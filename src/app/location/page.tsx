...
const Location = () => {
  ...
  useEffect(() => {
    if (results.length > 0) {
      ...
      setPersonajes(findEpisode.residents as string[])
      setLocation(findEpisode)
      if (findEpisode.residents?.length) {
        const pages = Math.ceil(findEpisode.residents?.length / 20)
        setPages(pages)
      }
      const fetchCharacters = async () => {
        try {
          ...
          const responses = await Promise.all(
            findEpisode.residents?.slice(0, 20).map((url) => fetch(url)) ?? []
          )
  ...
  return (
    <section className='flex flex-col items-center'>
      <HeadTitle
        ...
        dateDimension={location?.dimension}
        seasonType={location?.type}
      />
